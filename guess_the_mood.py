...
def mood_to_phrase_2gram(file):
    ...
    most_common_words_happy = defaultdict(lambda:4.2)
    most_common_words_sad = defaultdict(lambda:4.2)
...
def mood_to_phrase_3gram(file):
    mood_to_sent_count = defaultdict(float)
    most_common_words_happy = defaultdict(lambda:0.5)
    most_common_words_sad = defaultdict(lambda:0.5)
    openFile = open(file, "r", encoding="utf-8")
    readlines = openFile.readlines()
    print(len(readlines))
    for line in readlines:
...
datum2, mcwh2, mcws2 = mood_to_phrase_2gram("sentient_data.txt")
norm_dict2 = normalize_dict2(datum2)
datum3, mcwh3, mcws3 = mood_to_phrase_3gram("sentient_data.txt")
